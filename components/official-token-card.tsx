...
export function OfficialTokenCard(){
  ...
  return <aside className="relative min-w-0 overflow-hidden rounded-[1.75rem] border border-cyan/25 bg-[radial-gradient(circle_at_top_right,rgba(64,232,255,.18),transparent_45%),linear-gradient(145deg,rgba(255,255,255,.08),rgba(255,255,255,.025))] p-5 shadow-neon backdrop-blur sm:p-6">
    ...
    <div className="relative">
      ...
      <div className="mt-3 grid grid-cols-2 gap-3"><button type="button" onClick={copy} className="btn btn-primary min-h-11">{copied?'Copied ✓':'Copy address'}</button><a className="btn btn-ghost inline-flex min-h-11 items-center justify-center" href={market.data?.pairUrl||`https://basescan.org/token/${LIQPAD_TOKEN}`} target="_blank" rel="noreferrer">{market.data?.pairUrl?'View market ↗':'BaseScan ↗'}</a></div>
      <p className="mt-4 text-xs leading-5 text-muted">Market data via DEX Screener. Always verify the official contract address.</p>
    </div>
  </aside>
}
