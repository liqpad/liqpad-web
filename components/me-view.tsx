...
export function MeView({launches}:{launches:Launch[]}){
  const {address}=useAccount();const [visible,setVisible]=useState(12);useEffect(()=>setVisible(12),[address]);if(!address)return <div className="mx-auto max-w-3xl px-4 py-20 text-center"><h1 className="font-display text-4xl font-black">Your lantern is offline</h1><p className="mt-3 text-muted">Connect a wallet from the header to see your launches and creator fees.</p></div>;
